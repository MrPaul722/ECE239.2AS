...
class EnvWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        # Perform initial no-op steps
        for _ in range(self.initial_no_op):
            obs, _, terminated, truncated, info = self.env.step(self.do_nothing_action)
            if terminated or truncated:
                obs, info = self.env.reset(**kwargs)

        # Apply the provided preprocess function crop to 84x84, grayscale, normalize
        frame = preprocess(obs)

        # Stack the same frame 
        self.stacked_state = np.stack([frame] * self.stack_frames, axis=0)
    ...
    def step(self, action):
        ...
        # Update the frame stack: drop the oldest frame and append the new one
        self.stacked_state = np.concatenate(
            [self.stacked_state[1:], np.expand_dims(frame, 0)], axis=0
        )
