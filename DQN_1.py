...
class DQN:
    ...
    def _optimize_model(self):
        ...
        # compute current Q: q_values = self.model(states)
        
        q_values = self.model(states)  # shape: (batch_size, n_actions)
        state_action_values = q_values.gather(1, actions.unsqueeze(1)).squeeze(1)  # shape: (batch_size,)

        # compute target Q using the same model (no target network here)
        with torch.no_grad():
            next_q_values = self.model(next_states)       # shape: (batch_size, n_actions)
            max_next_q_values, _ = next_q_values.max(dim=1)  # shape: (batch_size,)

        # compute target Q: reward + gamma * max_next_q * (1 - done)
        target_q_values = rewards + self.gamma * max_next_q_values * (~dones)

        # compute loss between current Q and target Q
        loss = self.loss_fn(state_action_values, target_q_values)
    ...
    def _sample_action(self, state:np.ndarray
        ...
        if random.random() < epsilon:
            ...
            # Greedy action: choose argmax_a Q(s,a)
            state_tensor = torch.from_numpy(state).float().unsqueeze(0).to(self.device)  # shape: (1, n_c, H, W)
            with torch.no_grad():
                q_values = self.model(state_tensor)  # shape: (1, n_actions)
...
class HardUpdateDQN(DQN):
    ...
    def __init__(self,env,model,model_kwargs:dict = {},
        ...
        # Create a target_model as a copy of self.model and sync weights
    ...
    def _optimize_model(self):
        ...
        q_values = self.model(states)  # (batch_size, n_actions)
        state_action_values = q_values.gather(1, actions.unsqueeze(1)).squeeze(1)  # (batch_size,)

        # Compute target Q-values using the target_model (hard update network)
        with torch.no_grad():
            next_q_values = self.target_model(next_states)  # (batch_size, n_actions)
            max_next_q_values, _ = next_q_values.max(dim=1)  # (batch_size,)

        # Compute expected Q-values (targets)
        target_q_values = rewards + self.gamma * max_next_q_values * (~dones)
        ...
        # Hard‐update the target network every self.update_freq updates
        self._update_model()
