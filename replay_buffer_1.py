...
class ReplayBufferDQN:
    ...
    def sample(self, batch_size:int, device='cpu'):
        ...
        states = torch.stack([
            torch.from_numpy(exp[0]).float() for exp in experiences
        ]).to(device)

        actions = torch.tensor(
            [exp[1] for exp in experiences], 
            dtype=torch.int64
        ).to(device)

        rewards = torch.tensor(
            [exp[2] for exp in experiences], 
            dtype=torch.float32
        ).to(device)

        next_states = torch.stack([
            torch.from_numpy(exp[3]).float() for exp in experiences
        ]).to(device)

        dones = torch.tensor(
            [exp[4] for exp in experiences], 
            dtype=torch.bool
        ).to(device)
